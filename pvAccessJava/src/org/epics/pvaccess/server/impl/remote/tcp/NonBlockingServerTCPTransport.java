 ...
 * @version $Id$
 */
public class NonBlockingServerTCPTransport extends NonBlockingTCPTransport
    ...
    public NonBlockingServerTCPTransport(Context context,
        ...
        super(context, poller, channel, responseHandler, receiveBufferSize, PVAConstants.PVA_DEFAULT_PRIORITY);
        ...
        if (!verifyOrVerified) {
            ...
            for (SecurityPlugin securityPlugin : securityPlugins.values()) {
                try {
                    ...
                    context.getLogger().log(Level.SEVERE, "Unexpected exception caught while calling SecurityPlugin.isValidFor(InetAddress)/getId() methods.", th);
                }
            }
    ...
    private static final Status validationTimeoutStatus =
    ...
    private static final Status invalidSecurityPluginNameStatus =
            StatusFactory.getStatusCreate()
    ...
    public void authNZInitialize(Object data) {
        ...
        try {
            ...
            context.getLogger().log(Level.SEVERE, "Unexpected exception caught while calling SecurityPlugin.isValidFor(InetAddress) methods.", th);

            verified(StatusFactory.getStatusCreate()
                    .createStatus(StatusType.ERROR, "dysfunctional security plug-in", th));
            return;
        }
    ...
    public void authenticationCompleted(Status status) {
        ...
        context.getLogger().finer("Authentication completed with status '" + status.getType() + "' for PVA client: " + channel.socket().getRemoteSocketAddress());
    ...
    public void close() throws IOException {
        ...
        if (securitySession != null) {
            try {
                ...
                context.getLogger().log(Level.WARNING, "Unexpected exception caught while closing security session.", th);
            }
