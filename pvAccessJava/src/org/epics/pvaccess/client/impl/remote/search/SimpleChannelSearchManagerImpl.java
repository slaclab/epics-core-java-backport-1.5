...
public class SimpleChannelSearchManagerImpl implements ChannelSearchManager, TimerCallback, Runnable {
    ...
    private void initializeSendBuffer() {
        ...
        sendBuffer.putShort(responsePort);
     ...
     */
    private static boolean generateSearchRequestMessage(SearchInstance si, ByteBuffer requestMessage) {
        ...
        SerializeHelper.serializeString(name, requestMessage, SimpleChannelSearchManagerImpl.mockTransportSendControl);
     ...
     */
    private synchronized boolean generateSearchRequestMessage(SearchInstance channel) {
        boolean success = generateSearchRequestMessage(channel, sendBuffer);
        // buffer full, flush
        if (!success) {
            flushSendBuffer();
            generateSearchRequestMessage(channel, sendBuffer);
            return true;
        }
    ...
    private void send(SearchInstance[] sis) throws InterruptedException {
        ...
        for (SearchInstance si : sis) {
            ...
            if (generateSearchRequestMessage(si))
                frameSent++;
