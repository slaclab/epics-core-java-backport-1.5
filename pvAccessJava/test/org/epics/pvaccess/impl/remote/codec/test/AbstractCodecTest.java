...
public class AbstractCodecTest extends TestCase {
    ...
    static class TestCodec extends AbstractCodec {
        ...
        public void transferToReadBuffer() {
    ...
    public void testSplitAlignment() throws Throwable {
        ...
        @SuppressWarnings("unused") final TestCodec codec = new TestCodec(DEFAULT_BUFFER_SIZE);
        codec.readPayload = true;
        ...
        codec.readPollOneCallback = new ReadPollOneCallback() {

            public void readPollOne() {

                if (codec.readPollOneCount == 1) {
                    codec.readBuffer.clear();
                    for (int i1 = payloadSize1 - 2; i1 < payloadSize1; i1++)
                        codec.readBuffer.put((byte) i1);
                    // align
                    int aligned = AbstractCodec.alignedValue(payloadSize1, PVAConstants.PVA_ALIGNMENT);
                    for (int i1 = payloadSize1; i1 < aligned; i1++)
                        codec.readBuffer.put((byte) 0xFF);
                    ...
                    codec.readBuffer.put((byte) 0x45);
                    codec.readBuffer.putInt(payloadSize2);
                    for (int i1 = 0; i1 < payloadSize2; i1++)
                        codec.readBuffer.put((byte) i1);
                    codec.readBuffer.flip();
                    ...
                    for (int i1 = payloadSize2; i1 < aligned; i1++)
                        codec.readBuffer.put((byte) 0xFF);
                    codec.readBuffer.flip();
                } else
    ...
    public void testSegmentedInvalidInBetweenFlagsMessage() throws Throwable {
        ...
        codec.forcePayloadRead = payloadSize1 + payloadSize2 + payloadSize3 + payloadSize4;
    ...
    public void testSegmentedMessageAlignment() throws Throwable {
        ...
        for (int i = 0; i < payloadSize3; i++)
            codec.readBuffer.put((byte) (c++));
        aligned = AbstractCodec.alignedValue(payloadSize3, PVAConstants.PVA_ALIGNMENT);
        for (int i = payloadSize3; i < aligned; i++)
            codec.readBuffer.put((byte) 0xFF);
        ...
        for (int i = 0; i < payloadSize4; i++)
            codec.readBuffer.put((byte) (c++));
        aligned = AbstractCodec.alignedValue(payloadSize4, PVAConstants.PVA_ALIGNMENT);
        for (int i = payloadSize4; i < aligned; i++)
            codec.readBuffer.put((byte) 0xFF);
            ...
            for (int secondMessagePayloadSize = 0;
                ...
                for (int thirdMessagePayloadSize = 1;        // cannot be zero
                    ...
                    while (true) {
                        ...
                        codec.readBuffer.put((byte) 0x90);
                        codec.readBuffer.put((byte) 0x01);
                        codec.readBuffer.putInt(firstMessagePayloadSize);
                        int c = 0;
                        for (int i = 0; i < firstMessagePayloadSize; i++)
                            codec.readBuffer.put((byte) (c++));
                        int aligned = AbstractCodec.alignedValue(firstMessagePayloadSize, PVAConstants.PVA_ALIGNMENT);
                        for (int i = firstMessagePayloadSize; i < aligned; i++)
                            codec.readBuffer.put((byte) 0xFF);
                        ...
                        codec.readBuffer.put((byte) 0xB0);
                        codec.readBuffer.put((byte) 0x01);
                        codec.readBuffer.putInt(secondMessagePayloadSize);

                        for (int i = 0; i < secondMessagePayloadSize; i++)
                            codec.readBuffer.put((byte) (c++));
                        aligned = AbstractCodec.alignedValue(secondMessagePayloadSize, PVAConstants.PVA_ALIGNMENT);
                        for (int i = secondMessagePayloadSize; i < aligned; i++)
                            codec.readBuffer.put((byte) 0xFF);
                        ...
                        codec.readBuffer.put((byte) 0xA0);
                        codec.readBuffer.put((byte) 0x01);
                        codec.readBuffer.putInt(thirdMessagePayloadSize);

                        for (int i = 0; i < thirdMessagePayloadSize; i++)
                            codec.readBuffer.put((byte) (c++));
                        aligned = AbstractCodec.alignedValue(thirdMessagePayloadSize, PVAConstants.PVA_ALIGNMENT);
                        for (int i = thirdMessagePayloadSize; i < aligned; i++)
                            codec.readBuffer.put((byte) 0xFF);
                        ...
                        final int payloadSizeSum = firstMessagePayloadSize + secondMessagePayloadSize + thirdMessagePayloadSize;
                        codec.forcePayloadRead = payloadSizeSum;
            ...
            for (int secondMessagePayloadSize = 0;
                ...
                for (int thirdMessagePayloadSize = 1;        // cannot be zero
                    ...
                    for (int fourthMessagePayloadSize = 1;        // cannot be zero
                        ...
                        for (int i = 0; i < fourthMessagePayloadSize; i++)
                            codec.getSendBuffer().put((byte) (c++));
                        codec.endMessage();

                        codec.transferToReadBuffer();

                        final int payloadSizeSum = firstMessagePayloadSize + secondMessagePayloadSize + thirdMessagePayloadSize + fourthMessagePayloadSize;
                        codec.forcePayloadRead = payloadSizeSum;
            ...
            for (int secondMessagePayloadSize = 0;
                ...
                for (int thirdMessagePayloadSize = 1;        // cannot be zero
                    ...
                    while (true) {
                        ...
                        codec.forcePayloadRead = firstMessagePayloadSize + secondMessagePayloadSize + thirdMessagePayloadSize;
    ...
    public void testSendConnectionLoss() {
        final TestCodec codec = new TestCodec(DEFAULT_BUFFER_SIZE);
        codec.readPayload = true;
        codec.disconnected = true;
    ...
    public void testSendPerPart() throws Throwable {
    ...
    public void testClearSendQueue() {
        final TestCodec codec = new TestCodec(DEFAULT_BUFFER_SIZE);
    ...
    public void testBlockingProcessQueueTest() throws Throwable {
        ...
        assertFalse(processTreadExited.get());
        ...
        assertTrue(processTreadExited.get());
